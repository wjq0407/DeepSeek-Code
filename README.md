...
| **单元测试** | 零依赖 `node:test`，38 用例全绿（`npm test`）；含评测集结构静态校验（`npm run eval:check`，无需 API key）|
| **安全护栏** | `edit_file` 三级鲁棒匹配（fuzzyMatchBlock：精确 → 换行归一 → 逐行空白归一）+ 精确失败诊断；`run_command` 源码改写拦截（isSourceMutating）防「edit 失败后降级用命令改源码」|
...
| **CI** | 随仓库附带最小 GitHub Actions（`.github/workflows/ci.yml`：typecheck + node:test）；需 PAT 含 `workflow` 权限方可推送至 GitHub Actions 激活 |
...
### 开发脚本
```bash
npm run typecheck   # TS strict 类型检查（零错误）
npm test            # 运行 38 个 node:test 单元测试
npm run eval:check  # 静态校验评测集（23 case 结构合法，无需 API key）
```
...
### 已知改进项（诚实记录）
- `edit_file` 匹配失败后尚无「自动重读目标区间再重试」闭环，目前依赖 fuzzyMatchBlock 三级匹配 + `run_command` 拦截兜底（详见 eval/c18-c19-review.md）。
- 最小 CI 配置已随仓库附带，但需 PAT 含 `workflow` 权限方可推送至 GitHub Actions 激活（详见 `.github/workflows/ci.yml`）。
