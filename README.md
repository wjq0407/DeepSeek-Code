...
[![version](https://img.shields.io/badge/version-v0.4.0-blue)](./feature_list.json)
[![typescript](https://img.shields.io/badge/TypeScript-strict%200%20errors-3178c6)](./tsconfig.json)
[![eval](https://img.shields.io/badge/eval-20%20golden%20cases%20100%25-brightgreen)](./eval/RESULTS.md)
...
| **MCP 生态** | 成熟完整 | 函数式插件化雏形（可选后续）| 诚实说明：完整协议留作后续 |
...
| **Verification** | 20 黄金 case 三档评测 + 真实 API 联调 |
...
│ src/llm         │          │ 14 个工具 + Zod schema 校验    │
│ DeepSeek 原生    │          │ 基础 6 + Git 3 + 差异化 4 + 委派 │
...
**基础编程动作（6）**
- `read_file` / `create_file` / `edit_file` / `delete_file` / `run_command` / `search_code`
...
| **评测** | 20 黄金 case，三档验证（code 断言 / LLM 裁判 5 分 / 人工复核），**100% 通过** |
...
### 当前版本：v0.4.0（已收官）
P1–P5 全部完成，23 项功能全部 done，20 黄金 case 100% 通过，TS strict 零错误。

```
P1 DeepSeek 原生能力释放      ✅ 双模型路由 / 长上下文 / JSON mode
P2 Claude Code 优点集成       ✅ JSONL trace / Git 一等公民 / Plan Mode / Reflection
P3 差异化能力深化             ✅ terminology / project_discover
P4 工程质量加固               ✅ 子 Agent 委派 / 分级日志 / TS strict
P5 体验升级                   ✅ Markdown 渲染 / 会话恢复 / 成本估算
```

### 可选后续方向
- **完整 MCP 客户端协议**（SSE/stdio 传输 + JSON-RPC 握手 + 工具动态发现）——当前 `createTools` 函数式组装已是插件化雏形，完整协议可作为生态扩展。
